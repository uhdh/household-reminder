import { formatKRW, formatManwon, formatSignedPct } from "@/lib/finance-format";
...
export function SummaryCard({
  ...
  const breakdownText = breakdown
    .map((b) => `${b.label} ${format === "signedPct" ? formatSignedPct(b.value) : format === "manwon" ? formatManwon(b.value) : formatKRW(b.value)}`)
    .join(" · ");
  ...
  return (
    <div className={`seed-card shadow-none ${isFeature ? "flex flex-col justify-center px-6 py-6 sm:px-8 sm:py-8" : "px-5 py-4"} ${className}`}>
      ...
      <p
        title={format === "signedPct" ? formatSignedPct(value) : format === "manwon" ? formatManwon(value) : formatKRW(value)}
        className={
          isFeature
